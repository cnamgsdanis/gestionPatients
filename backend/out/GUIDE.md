...
API demarree sur http://localhost:8080
  ...
  GET/PUT/DELETE    /api/utilisateurs
...
> Copie le `token` et l'utilise en **Bearer Token** sur toutes les requêtes patients.
...
> Ne pas envoyer `matricule_nag` : il est **généré automatiquement** en INT **10 chiffres** (chiffres uniquement, pas de préfixe imposé).
...
```json
{ "id_patient": 1007, "matricule_nag": 2026000001 }
```

---

### 7. Requête 5 — Rechercher un patient par NAG

| Champ | Valeur |
|---|---|
| **Méthode** | `GET` |
| **URL** | `{{baseUrl}}/api/patients/nag/2026000001` |
| **Auth** | Bearer Token |
| **Body** | aucun |

**Réponse attendue (200 OK)** : objet patient complet.

Si le NAG contient des lettres ou n’a pas 10 chiffres → **400**.

---

### 8. Requête 6 — Modifier un patient

| Champ | Valeur |
|---|---|
| **Méthode** | `PUT` |
| **URL** | `{{baseUrl}}/api/patients/1007` |
| **Auth** | Bearer Token |
| **Onglet Body** | `raw` → type **JSON** |
...
> Le NAG **n'est jamais modifié** par le PUT.
...
### 9. Requête 7 — Supprimer un patient
...
| GET     | `/api/patients/nag/{nag}`  | Patient par NAG (INT, 10 chiffres uniquement) |
| POST    | `/api/patients`            | Créer (NAG auto INT)                 |
| PUT     | `/api/patients/{id}`       | Modifier (NAG protégé)               |
...
- [x] Ajouter une session par token (JWT) pour protéger les routes
- [x] NAG patient en INT auto + recherche `/api/patients/nag/{nag}`
- [ ] Ajouter les autres entités (`Structure`, `Prestation`, `Ordonnance`…)
- [ ] Brancher le frontend sur l'API réelle
